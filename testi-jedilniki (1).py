...
def ena_jed(jed, jedcev):
    nov = {}
    for sestavina, koliko in jedi[jed].items():
        nov[sestavina] = jedcev * koliko
    return nov


def nakup(obroki):
    kaj_kupiti = collections.defaultdict(float)
    for jed, jedcev in obroki:
        for sestavina, koliko in ena_jed(jed, jedcev).items():
            kaj_kupiti[sestavina] += koliko
    return kaj_kupiti


def obrokov(jed, zaloga):
    koliko = set()
    for sestavina, koliko_potrebujemo in jedi[jed].items():
        koliko.add(zaloga[sestavina] // koliko_potrebujemo)
    return min(koliko)
